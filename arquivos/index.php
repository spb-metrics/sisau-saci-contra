<?
/****************************************************************************
*  Sistema   : SACI - Sistema de Administração de Conteúdos na Internet		*
*  Descrição : Apresentação dinâmica de conteúdos SACI						*
...
define( 'arqconfig', "/var/www/sistemas/config.php" );
...
* @param $arquivo
* @param $variavel
* @desc Retorna o valor do parâmetro setado no arquivo de configurações de sistema
*/
...
// Verifica se o arquivo de configuraçao setado existe
if ( !file_exists( constant( 'arqconfig' ) ) ) {
        $msg = "Não foi possível encontrar o arquivo de configuração do portal!";
        exit();
}
...
// Faz a leitura das variáveis setadas no arquivo de configuração
$config = array( 'dirraiz'    => get_var_config( $conf, 'dirraiz' ),    // Diretório raiz onde os scripts estão armazenados no servidor
                                 'dirsis'     => get_var_config( $conf, 'dirsis' ),             // Diretório onde estão armazenados os scripts de sistema
                                 'dirphppad'  => get_var_config( $conf, 'dirphppad' ),  // Diretório de scripts padrão dos sistemas
                                 'bdhost'     => get_var_config( $conf, 'bdhost' ),     // Host servidor de banco de dados
                                 'bdsrvprt'   => get_var_config( $conf, 'bdsrvprt' ),   // Porta de conexão do servidor de banco de dados
                                 'bdnome'     => get_var_config( $conf, 'bdnome' ),     // Nome do banco de dados dos sistemas
								 'bdusrlogin' => get_var_config( $conf, 'bdusrlogin' ), // Login do usuário dos sistemas no banco de dados
                                 'bdusrpass'  => get_var_config( $conf, 'bdusrpass' )   // Senha do usuário dos sistemas no banco de dados
);
...
$dmnid          = db_ret_val( $res_dmn, 0, 'dmnid' );		// Pega o identificador do domínio
$dir_img_upload = db_ret_val( $res_dmn, 0, 'dmnlocup' );	// Pega o local pra uploads de arquivos do domínio
$script 		= db_ret_val( $res_dmn, 0, 'mopscript' ); 	// Pega o script do template da seção principal do domínio
$dmnsccid       = db_ret_val( $res_dmn, 0, 'dmnsccid' );        // Pega a seção principal do domínio

if ( intval( $sccid ) > 0 ) {
	// Pega o identificador do domínio acessado e seção inicial
    $res_script = db_select( $dbc, "select mopscript from scc join mop on 
    								mopid=sccmopidvis and sccsts='t' and 
    								sccid=$sccid
    " );
    
    $script = db_ret_val( $res_script, 0, "mopscript" ); 	// Pega o script do template da seção atual
} else {
	$sccid  = db_ret_val( $res_dmn, 0, "dmnsccid" ); 		// Pega o identificador da seção principal do domínio
}
